...
-- -----------------------------------------------------------------
-- Modified: skip Latin spans/divs (lang="la")
-- -----------------------------------------------------------------
local function checkspan(el)
  local lang = el.attributes.lang
  -- If there is no language attribute, or the language is Latin, do nothing.
  if not lang or lang == "la" then
    return nil                -- keep the span unchanged and do not spell‑check it
  end
  pandoc.walk_inline(el, {Str = function(e) add_to_dict(lang, e.text) end})
  return {}                    -- remove the span so it isn’t checked again
end

local function checkdiv(el)
  local lang = el.attributes.lang
  -- Same logic as for spans: ignore Latin blocks.
  if not lang or lang == "la" then
    return nil                -- keep the div unchanged and do not spell‑check it
  end
  pandoc.walk_block(el, {Str = function(e) add_to_dict(lang, e.text) end})
  return {}                    -- remove the div so it isn’t checked again
end
